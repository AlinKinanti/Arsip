...
class ControllerSurat extends CI_Controller {
  ...
  public function daftarSuratMasuk(){
    if($this->session->userdata('statusLogin') == "loginAdmin"){
      ...
      $data['itemDisposisi'] = $this->m_disposisi->getAllDiposisi();
      ...
      $data['itemSurat'] = $this->m_surat->readSurat(FALSE, FALSE, $where);
      $data['itemDisposisi'] = $this->m_disposisi->getAllDiposisi();
      $this->load->view('kepala/k_suratMasuk', $data);
      $this->load->view('footer');
    }
    else{
      redirect(base_url(''));
  ...
  public function tambahSuratMasuk(){
    if($this->session->userdata('statusLogin') != "loginAdmin"){
      ...
      if (!$this->upload->do_upload('fileSurat')){
        ...
        $noSurat = $this->m_surat->getIdSurat(['noSurat'=>$this->input->post('noSuratTS')]);
       
        // auto mencreate disposisi
        $this->load->model('M_disposisi');
        $this->M_disposisi->createDisposisi($noSurat[0]->id);
  ...
  public function daftarSuratKeluar(){
    if($this->session->userdata('statusLogin') == "loginAdmin"){
      ...
      $data['itemSurat'] = $this->m_surat->readSurat(FALSE, FALSE, $where);
      $this->load->view('kepala/k_suratKeluar', $data);
      $this->load->view('footer');
    }
    else{
      redirect(base_url(''));
  ...
  public function print_suratmasuk()
  {
    $date = $this->input->post('datePrint');
    $where = ["DATE_FORMAT(tgl,'%Y-%m')"=>$date];
    $data['itemSurat'] = $this->m_surat->readSurat(1, FALSE, $where);
    $this->load->view('admin/a_csmasuk', $data);
  }

  public function print_suratkeluar()
  {
    $date = $this->input->post('datePrint');
    $where = ["DATE_FORMAT(tgl,'%Y-%m')"=>$date];
    $data['itemSurat'] = $this->m_surat->readSurat(2, FALSE, $where);
    $this->load->view('admin/a_cskeluar', $data);
  }
  ...
  public function disposisi(){
    if($this->session->userdata('jabatan') == 0 ){
      redirect('ControllerPegawai');
    }    
    $data['itemDisposisi'] = $this->m_disposisi->getAllDiposisi();
    $this->load->view('kepala/k_disposisi', $data);
  }

  public function updateDisposisi(){
    if($this->session->userdata('jabatan') == 0 ){
      $this->M_disposisi->updateDisposisi($id_surat, $data);
      redirect('ControllerSurat/disposisi');
      redirect('ControllerPegawai');
    }    
    ...
    if(is_null($checkboxBidang)){
      ...
      //bidang diisi berdasar checkbox
      $bidang = implode(",", $checkboxBidang);
    }

    // update data untuk checkbox berdasarkan id_surat
    $this->load->model('M_disposisi');
    $data = $this->input->post();
    $data['tujuan']=$bidang;
    $id_surat = $data['id_surat'];

    //memperbaharui data yang ada di form
    unset($data['id_surat']);
    unset($data['check_list1']);

    //seleksi status disposisi selesai atau belum
   ...
   public function viewDisposisi($id){
    $data['itemDisposisi'] = $this->m_disposisi->getDiposisiByMessageNumber($id);
    $this->load->view('admin/a_formDisposisi', $data);
   }

   public function daftarDisposisi(){
    if($this->session->userdata('statusLogin') == "loginAdmin"){
      $data['itemDisposisi'] = $this->m_disposisi->getAllDiposisi();
      $this->load->view('admin/a_daftarDisposisi', $data);
      $this->load->view('footer');
    }
    else if($this->session->userdata('statusLogin') == "loginUser"){
      $where = array(
      ...
      $data['itemDisposisi'] = $this->m_disposisi->getAllDiposisi();
      $this->load->view('pegawai/u_daftarDisposisi', $data);
      $this->load->view('footer');
    }
    else{
      redirect(base_url(''));
